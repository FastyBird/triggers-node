...
final class TriggersV1ControllerTest extends DbTestCase
{
	...
	public function testCreate(string $url, ?string $token, string $body, int $statusCode, string $fixture): void
	{
		...
		$rabbitPublisher
			...
			->withArgs(function (string $routingKey, array $data): bool {
				Assert::same('fb.bus.node.entity.created.trigger', $routingKey);
				Assert::false($data === []);
	...
	public function testUpdate(string $url, ?string $token, string $body, int $statusCode, string $fixture): void
	{
		...
		$rabbitPublisher
			->shouldReceive('publish')
			->withArgs(function (string $routingKey, array $data): bool {
				Assert::same('fb.bus.node.entity.updated.trigger', $routingKey);
				Assert::false($data === []);
		...
		$rabbitPublisher
			...
			->withArgs(function (string $routingKey, array $data): bool {
				Assert::true(in_array($routingKey, [
					...
					'fb.bus.node.entity.deleted.trigger.notification',
				], true));
				Assert::false($data === []);
